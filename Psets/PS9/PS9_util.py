...
class AbstractReliableSenderNode(Router):
    ...
    def OnClick(self,which):
        if which == 'left':
            if self.network.time > 1:
                print(repr(self) + "srtt %.1f rttdev %.1f timeout %.1f" % (self.srtt, self.rttdev, self.timeout))
            else:
                print(repr(self))
    ...
    def receive(self,p,link,time):
        if p.type == 'ACK':
            ...
            self.process_ack(acknum, pkt_timestamp)
    ...
    # Implemented by ReliableSenderNode
    def process_ack(self, acknum, timestamp):
        pass

    # Implemented by ReliableSenderNode
    def calc_timeout(self, pkt_timestamp):
        pass

    # Implemented by ReliableSenderNode
    def send_packet(self, time, seqnum, pkt_timestamp):
        pass
...
class AbstractReliableReceiverNode(Router):
    ...
    def OnClick(self,which):
        if which == 'left':
            if self.network.time > 1:
                print(repr(self) + " received %d (%.2f packets/timestep)" % (self.app_seqnum, float(self.app_seqnum)/(self.network.time - 1)))
            else:
                print(repr(self))
    ...
    def receive(self, p, link, time):
        ...
        if p.type == 'DATA':
            self.reliable_recv(p.source, seqnum, pkt_timestamp)
