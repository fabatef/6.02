...
class HuffmanTree:
    ...
    def build_tree(self):
        ...
        min_heap = [HuffmanNode(syb, self.source[syb]) for syb in self.source]
        heapq.heapify(min_heap)
        while len(min_heap) != 1:
            lc, rc = heapq.heappop(min_heap), heapq.heappop(min_heap)
            parent = HuffmanNode(None, lc.probability + rc.probability)
            parent.left_child, parent.right_child  = lc, rc
            heapq.heappush(min_heap, parent)
            
        self.root = heapq.heappop(min_heap)
